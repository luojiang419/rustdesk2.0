...
fn ping_host_for_endpoint(endpoint: &str) -> Option<String> {
    let mut endpoint = normalize_server(endpoint);
    ...
    if let Some((_, rest)) = endpoint.rsplit_once('@') {
        endpoint = rest.to_owned();
    }
    if let Some((host, _)) = endpoint.split_once('/') {
...
fn parse_ping_latency_ms(output: &str) -> Option<i64> {
    let bytes = output.as_bytes();
    let mut pos = 0;
    while pos + 1 < bytes.len() {
        if bytes[pos].eq_ignore_ascii_case(&b'm') && bytes[pos + 1].eq_ignore_ascii_case(&b's') {
            if let Some(value) = parse_number_before_ms(&output[..pos]) {
                return Some(value);
            }
            pos += 2;
        } else {
...
fn parse_number_before_ms(input: &str) -> Option<i64> {
    ...
    let value = input[start..end].replace(',', ".");
    let value = value.parse::<f64>().ok()?;
    if !value.is_finite() || value < 0.0 {
        return None;
    }
    Some((value.ceil() as i64).max(1))
}

fn elapsed_millis_at_least_one(start: Instant) -> i64 {
...
mod tests {
    ...
    fn extracts_ping_host_from_endpoint() {
        ...
        assert_eq!(
            ping_host_for_endpoint("https://user:pass@example.com:21117/path").as_deref(),
            Some("example.com")
        );
        assert_eq!(
            ping_host_for_endpoint("2001:db8::2").as_deref(),
            Some("2001:db8::2")
        );
    }
    ...
    fn parses_ping_latency_outputs() {
        ...
        assert_eq!(
            parse_ping_latency_ms("Reply from 127.0.0.1: bytes=32 time<1ms TTL=128"),
            Some(1)
        );
        assert_eq!(parse_ping_latency_ms("Request timed out."), None);
    }
}
